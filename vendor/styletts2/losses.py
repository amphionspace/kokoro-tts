...
def discriminator_TPRLS_loss(disc_real_outputs, disc_generated_outputs):
    ...
    for dr, dg in zip(disc_real_outputs, disc_generated_outputs):
        ...
        # BF16 ties can leave the selected set empty; its contribution is zero.
        residual = (dr.float() - dg.float()) - m_DG.float()
        selected = dr < dg + m_DG
        L_rel = (residual.square() * selected).sum() / selected.sum().clamp_min(1)
