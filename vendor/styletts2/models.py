        ...
        for _ in range(nlayers):
            self.lstms.append(
                nn.LSTM(
                    ...
                    batch_first=True,
                    bidirectional=True,
                    # One layer has no inter-layer dropout. A nonzero value
                    # still allocates cuDNN dropout RNG state on first forward,
                    # which breaks resumed RNG equivalence. Explicit F.dropout
                    # below retains the intended regularization.
                    dropout=0.0,
                )
            )
