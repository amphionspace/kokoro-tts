...
当前评价调用 `forward_with_tokens`，输入在前端已严格校验。没有使用会静默过滤未知字符的通用字符串入口。单次最多 510 个有效音素；当前评价不会静默截断超长文本。面向长文章的自动分句、拼接和停顿策略还没有在本项目中实现。
...
同步验证、配对诊断和固定文本自由合成承担不同职责。

### 10.0 配对诊断与试听报告

每个非 smoke checkpoint 的异步评价还执行 `scripts/run_checkpoint_diagnostics.py`：全部 400 条验证音频按训练路径做确定性片段重建，比较参考 style 与固定声学 style 的谱/WavLM 损失，并统计软对齐、MAS 时长和波形峰值。

固定每语言 8 条时长分位样本，生成完整句子的原音频、真实对齐/F0/能量+参考 style、同条件+固定声学 style、固定 voice 自由合成；Stage 2 另加真实对齐+预测 F0/能量的参考/固定 style 两组。仅对完整句子跑 LITs ASR/相似度/DNSMOS，禁止把片段与整句文本比较。生成 `diagnostics/<checkpoint>/report.html`，带音频和对齐图，并写入 TensorBoard。24 条是定位问题的诊断子集，不作为完整语料质量估计。
